	<?php $this->load->view('templates/user/header'); ?>
		<!--_______________________________________ Beranda__________________________________ -->
		<div class="allcontain">
			<div id="carousel-up" class="carousel slide" data-ride="carousel">
				<div class="carousel-inner " role="listbox">
					<div class="item active">
						<center>
							<img src="<?php echo base_url();?>assets/image/depan1.png"  width="1403" height="672">
							<div class="carousel-caption">
								<h1>SELAMAT DATANG DI InKost</h1>
								<h2> Cirebon, Jawa Barat</h2>
							</div>
						</div>

						<div class="item">
							<img src="<?php echo base_url();?>assets/image/dalam.jpg"  width="1403" height="472">
							<div class="carousel-caption">
								<h1>Dapatkan Hunian Nyaman hanya di InKost</h1>
								
							</div>
						</div>

						<div class="item">
							<img src="<?php echo base_url();?>assets/image/luar.jpg"  width="1403" height="472">
							<div class="carousel-caption">
								<h1>Harga Terjangkau, Free Wifi dan Free Fasilitas</h1>
							</center>
							</div>
						</div>
				</div>>
				<nav class="navbar navbar-default midle-nav">
					<div class="navbar-header">
						<button type="button" class="navbar-toggle collapsed textcostume" data-toggle="collapse" data-target="#navbarmidle" aria-expanded="false">
							<h1>SEARCH TEXT</h1>
							<span class="sr-only">Toggle navigation</span>
							<span class="icon-bar"></span>
							<span class="icon-bar"></span>
							<span class="icon-bar"></span>
						</button>
					</div>
					<br>
					<br>
					<br>

			</div>
		</nav>
	</div>
</div>
<?php $this->load->view('templates/user/footer'); ?>